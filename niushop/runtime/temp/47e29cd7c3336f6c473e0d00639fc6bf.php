...
<div class="template-message"><h1></h1><p style="width:90%;margin: 9px 7px 10px;">短信通知模板</p></div>
	<div class="tab-pane active">
		...
		<div class="right_main">
			...
			<div class="qianming"><div style="width:100px;float:left;">模板ID：</div><input type="text" id="template_titleid" value="<?php echo $template_select['template_title']; ?>" style="margin-right: 15px;height: 30px;line-height: 30px;" class="input-common" /><span style="font-size:13px;">必须与阿里大鱼的模板ID一致，否则无效!</span></div>
			<div class="qianming"><div style="width:100px;float:left;">短信签名：</div><input type="text" id="signNameid" style="margin-right: 15px;height: 30px;line-height: 30px;" value="<?php echo $template_select['sign_name']; ?>" class="input-common" /><span style="font-size:13px;">与阿里大鱼内短信签名保持一致!</span></div>
			<div class="qianming"><div style="width:100px;float:left;">通知人手机号：</div><input type="text" id="notification_mode" style="margin-right: 15px;height: 30px;line-height: 30px;" value="<?php echo $template_select['notification_mode']; ?>" class="input-common" /><span style="font-size:13px;">请输入通知人手机号，多个手机之间用英文逗号“,”隔开</span></div>
			<div class="bl">
...
function template_onclick(event){
	...
	for($i=0; $i<$template_array.length;$i++){
		...
		if($template_obj_type==$template_type){
			...
			$("#notification_mode").val($template_obj.notification_mode);
		}
	}
	$is_first=0;
...
function template_submit(){
	...
	$.ajax({
		...
		data: { "type": "sms", "template_data": JSON.stringify($template_array),"notify_type" : "business" },
		async : false, // 让它同步执行
		success: function (data) {
			if (data["code"] > 0) {
				showMessage('success', '提交成功');
				window.location.href=__URL("PLATFORM_MAIN/config/businessnotifytemplate?type=sms");
			}else{
