...
//店铺提现列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/shop/shopaccountwithdrawlist'); ?>",
			async : true,
			data : {
	...
	function Andit(id,shop_id,status){
                    ...
                    $.ajax({
            			type : "post",
            			url : "<?php echo __URL('PLATFORM_MAIN/shop/shopaccountwithdrawaudit'); ?>",
            			async : true,
            			data : {
            				"id" : id,
            				"shop_id" : shop_id,
            			...
            			success : function(data) {
            				//alert(JSON.stringify(data));
            				if(data["code"] > 0 ){
            					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/shop/shopaccountwithdrawlist'); ?>");
            				}else{
