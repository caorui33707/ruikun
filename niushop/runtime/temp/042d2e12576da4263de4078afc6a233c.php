...
<div class="row padder-v">
	<div class="col-sm-9">
		<a href="<?php echo __URL('PLATFORM_MAIN/Shop/addShopLevel'); ?>"
...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/Shop/shopLevelList'); ?>",
			async : true,
			data : {
			...
			success : function(data) {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="center"><a href="'+ __URL('PLATFORM_MAIN/Shop/updateShopLevel?instance_typeid='+ data['data'][i]['instance_typeid'])+'">修改</a></td>';
						html += '</tr>';
					}
				} else {
