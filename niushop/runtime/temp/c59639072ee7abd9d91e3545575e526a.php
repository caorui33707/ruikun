...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/cms/topiclist'); ?>",
		async : true,
		data : {
		...
		//	alert(JSON.stringify(data));
			...
			if (data["data"].length > 0) {
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><a target="_blank" href="'+__URL('SHOP_MAIN/topic/detail?topic_id='+ data['data'][i]['topic_id'])+'">预览</a>&nbsp;&nbsp;<a href="PLATFORM_MAIN/cms/updatetopic?id='+ data['data'][i]['topic_id']+'">修改</a>&nbsp;&nbsp;<a style="cursor: pointer;" onclick="deleteTopic('+data['data'][i]['topic_id']+')">删除</a></td> ';
					html += '</tr>';
				}
			} else {
		...
		buttons: {
			"确定": function() {
				...
				if(!flag){
					...
					$.ajax({
						type : "post",
						url : "<?php echo __URL('PLATFORM_MAIN/cms/deletetopic'); ?>",
						data : {
						...
						success : function(data) {
							if (data["code"] > 0) {
								...
								location.href = "<?php echo __URL('PLATFORM_MAIN/cms/topiclist'); ?>";
							}else{
