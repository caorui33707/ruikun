<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:43:"template/adminblue\Account\shopAccount.html";i:1513303780;s:28:"template/adminblue\base.html";i:1513303780;s:45:"template/adminblue\controlCommonVariable.html";i:1513303780;s:32:"template/adminblue\urlModel.html";i:1513303781;s:34:"template/adminblue\pageCommon.html";i:1513303781;s:34:"template/adminblue\openDialog.html";i:1513303780;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
	...
	<div class  =  'count-top'>
		<div class="count-top-title">
			...
			<div class="count-top-text">
				...
				<button class="count-button" onclick="window.location.href='<?php echo __URL('ADMIN_MAIN/Account/applyShopAccountWithdraw'); ?>'" >提现</button>			
			</div>
		</div>
...
function LoadingInfo(pageIndex) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/account/shopOrderAccountList'); ?>",
		async : true,
		data : {
			"pageIndex" : pageIndex,
		...
		success : function(data) {
			...
			if (data.list["data"].length > 0) {
				for (var i = 0; i < data.list["data"].length; i++) {
					...
					html += '<td>' + timeStampTurnTime(data.list["data"][i]["create_time"]) + '</td>';
...
function getShopAccountWithdrawPage(pageIndex){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/account/shopAccountWithdrawList'); ?>",
		async : true,
		data : {
			"pageIndex" : pageIndex,
			"start_date":start_date,
		...
		success : function(data) {	
			...
			if (data["list"]["data"].length > 0) {
				for (var i = 0; i < data["list"]["data"].length; i++) {
					...
					html += '<td>' + timeStampTurnTime(data["list"]["data"][i]["ask_for_date"]) + '</td>';
					if(data["list"]["data"][i]["payment_date"] == null){
						html += '<td> </td>';
					}else{
						html += '<td>' + timeStampTurnTime(data["list"]["data"][i]["payment_date"]) + '</td>';
					}					
					html += '<td>' + status + '</td>';						
					html += '</tr>';
				}
...
function getShopAccountRecordCount(pageIndex){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/Account/ShopAccountRecordCount'); ?>",
		async : true,
		data : {
