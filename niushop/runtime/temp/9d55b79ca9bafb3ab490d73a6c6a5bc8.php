...
function delcache() {
  ...
  <script type="text/javascript">
    ...
    if(verify(nav_title,nav_url)){  
    	$.ajax({
    		type:"post",
    	    url:"<?php echo __URL('PLATFORM_MAIN/config/addshopnavigation'); ?>",
    	    data:{
    	      'nav_title':nav_title,   
    	      'sort':sort,
    	      'nav_url':nav_url,
    	    ...
    	    success:function(data){
    	    	 if (data["code"] > 0) {
    						showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/shopnavigationlist'); ?>");
    					}else{
