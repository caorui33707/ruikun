...
function selectCityListAjax(province_id){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/config/selectcitylistajax'); ?>",
...
function selectDistrictListAjax(province_id,city_id){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/config/selectdistrictlistajax'); ?>",
	...
	$( "#dialog" ).dialog({
		buttons: {
			...
			$(this).dialog('close');
				$.ajax({
					type:"post",
					url:"<?php echo __URL('PLATFORM_MAIN/config/deleteregion'); ?>",
					data:{
						'regionId':regionId,
						'type' : type
					},
					...
					success:function (data) {
						if(data['code'] > 0){
							showMessage("success", "删除成功", "<?php echo __URL('PLATFORM_MAIN/config/areamanagement'); ?>");
						}else{
							$("#dialog").dialog({
								 buttons: {
									"确定,#e57373": function() {
...
function addOrUpdateAjax(){
	...
	var area_id = 0;
	if(type ==1){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/addcityajax'); ?>";
	}
	if(type ==2){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/adddistrictajax'); ?>";
	}
	if(type ==3){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/updateprovinceajax'); ?>";
		area_id = $("#area_select").val();//所属区域，只有一级地区有;
	}
	if(type ==4){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/updatecityajax'); ?>";
	}
	if(type ==5){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/updatedistrictajax'); ?>";
	}
	if(type == 6){
		var ajaxUrl = "<?php echo __URL('PLATFORM_MAIN/config/addprovinceajax'); ?>";
		area_id = $("#area_select").val();//所属区域，只有一级地区有;
	}
	...
	$.ajax({
		...
		success : function(data){
			...
			if(data['code']>0){
				...
				location.href = "<?php echo __URL('PLATFORM_MAIN/config/areamanagement'); ?>";
			}else{
...
function updateRegionAjax(regionType,upType,event){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/config/updateregionajax'); ?>",
		data : {
