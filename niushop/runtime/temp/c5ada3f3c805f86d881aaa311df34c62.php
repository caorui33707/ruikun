...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/Shop/shopApplyList'); ?>",
			async : true,
			data : {
...
// 						html += '<td class="center">' + data["data"][i]["user_name"] + '</td>';
						...
						if(data["data"][i]['apply_state'] == 1){
							html += '<td class="center"><a href="'+ __URL('PLATFORM_MAIN/shop/shopVerify?id='+ data["data"][i]['apply_id'])+'">查看</a>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="save(1,'+ data["data"][i]['apply_id']+')">通过</a>&nbsp;&nbsp;<a href="javascript:void(0);" onclick="save(2,'+ data["data"][i]['apply_id']+')">拒绝</a></td>';
						}else if(data["data"][i]['apply_state'] == 2){
							html += '<td class="center"><a href="'+ __URL('PLATFORM_MAIN/shop/shopVerify?id='+ data["data"][i]['apply_id'])+'">查看</a></td>';
						}else if(data["data"][i]['apply_state'] == -1){
							html += '<td class="center"><a href="'+ __URL('PLATFORM_MAIN/shop/shopVerify?id='+ data["data"][i]['apply_id'])+'">查看</a></td>';
						}
