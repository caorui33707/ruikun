...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/config/linklist'); ?>",
			async : true,
			data : {
	...
	$( "#dialog" ).dialog({
        buttons: {
            "确定": function() {
                ...
                $.ajax({
    				type:"post",
    				url:"<?php echo __URL('PLATFORM_MAIN/config/dellink'); ?>",
    				data:{'link_id':link_id},
    				async:true,
    				success:function (data) {
    					if(data['code'] > 0){
    						location.href = "<?php echo __URL('PLATFORM_MAIN/config/linklist'); ?>";
    					}else{
    						$("#dialog").dialog({
    							 buttons: {
    					                "确定,#e57373": function() {
