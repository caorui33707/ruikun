...
function updateShopApplyAjax(){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/shop/updateshopapply'); ?>",
		data : {
			'apply_id' : apply_id,
		...
		success : function(data) {
			if (data["code"] > 0) {
				showMessage('success', "编辑成功",
						__URL('PLATFORM_MAIN/Shop/updateShop?shop_id='
								+ shop_id));
			} else {
	...
	function updateShopAjax() {
		...
		if(shop_name == ''){
    		$("#shop_name").parent().next(".error").show();
      		$("#shop_name").focus();
       		return false;
		}
		$.ajax({
					type : "post",
					url : "<?php echo __URL('PLATFORM_MAIN/shop/updateshop'); ?>",
					data : {
						'shop_id' : shop_id,
						'shop_name' : shop_name,
