			...
			<div class="tab-pane fade active in" id="wpay">
					<div class="panel-body">
						<form class="form-horizontal" id="wpay_form"  method="post">
							...
							<div class="line line-dashed line-lg pull-in"></div>
							<div class="form-group">
								<label class="col-lg-2 control-label">支付授权目录:</label>
								<div class="col-md-4">
									<span class="text" style="background-color: white; border: none;min-width: 500px;display: inline-block;font-size: 13px;cursor: pointer;color:#468847;" id="empowerUrl" value=""><?php echo $root_url; ?></span>
								</div>
								<label class="col-md-6 error" for="MCHID" class="error"></label>
							</div>
							<div class="line line-dashed line-lg pull-in"></div>
			...
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"]);
					 location.href = "<?php echo __URL('PLATFORM_MAIN/config/payconfig'); ?>";
				}else{
 ...
 */
	function setAliPayConfigAjax(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/config/setalipayconfig'); ?>",
			data:{
			...
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/payconfig'); ?>");
				}else{
					showMessage('error', data["message"]);
				}	
			}
	 ...
	 */
		function setopayConfigAjax() {
			...
			$.ajax({
				type : "post",
				url : "<?php echo __URL('PLATFORM_MAIN/config/setopayconfig'); ?>",
				data : {
