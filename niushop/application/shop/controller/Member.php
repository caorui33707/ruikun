...
use data\model\SysUserAdminModel as SysUserAdminModel;
...
class Member extends BaseController
{
    ...
    public function userAgent(){
        ...
        $sysUserAdminObj = new   SysUserAdminModel();
        if (request()->isPost()) {
            ...
            $data = array(
                ...
                'admin_status'=>1,
                'is_agent'=>1,
                'group_id_array'=>12,//代理商管理
            );

            $systemData =  array(
                'is_system'=>1
            );
            if($sysUserAdminObj->getCount(array('uid'=>$this->uid))) {
                $res = $sysUserAdminObj->save($data, array('uid' => $this->uid));
            }else{
                $res = $sysUserAdminObj->save($data);
            }

            $this->user->updateSystemByUid($this->uid);

            return AjaxReturn(1);
        }
        $condition = array('uid'=>$this->uid);
        $data = $sysUserAdminObj->getInfo($condition,'admin_name');

        $member_detail = $this->user->getMemberDetail($this->instance_id);
        $this->assign("member_detail", $member_detail);

        $this->assign("user_agent", $data);
        return view($this->style . "Member/userAgent");
    }
    ...
    public function agentList(){
       ...
       //  $this->assign('path_info', 'member/agentList');


        $statusArray = array('未知','审核中','审核失败','审核成功');

        $sysAgentObj = new  SysAgentModel();
        $member_detail = $this->user->getMemberDetail($this->instance_id);
        $this->assign("member_detail", $member_detail);

        $where = ['uid'=>$this->uid,'is_default'=>0];
        $agentlist = $sysAgentObj->getInfo($where,'*');

        $total_count = $sysAgentObj->getCount($where);

        if($agentlist) {
            $is_status = $agentlist['is_status'];
            $agentlist['status_val'] = $statusArray[$is_status];

            $address = new Address();
            $address_info = $address->getAddress($agentlist['province_id'], $agentlist['city_id'], $agentlist['district_id']);
            $agentlist['agent_info'] = $address_info;
        }


        $this->assign('total_count',$total_count);
        $this->assign('list', $agentlist);

        return view($this->style . "Member/agentList");
    }
    ...
    public function agentSave(){
        ...
        if($this->request->isAjax()){
            ...
            $agentlist = $sysAgentObj->getInfo(['uid'=>$this->uid,'is_default'=>0],'*');


            if($agentlist['is_status'] == 3){
                return AjaxReturn(0);
            }
            ...
            if($id){
                $res = $sysAgentObj->save($data,['id'=>$id,'uid'=>$this->uid]);
            }else{
        ...
        $member_detail = $this->user->getMemberDetail($this->instance_id);
        $this->assign("member_detail", $member_detail);
        return view($this->style . "Member/agentinsert");
    }
    ...
    public function agentUpdate(){
        ...
        $this->assign('agent_id', $agentlist['id']);

        return view($this->style . "Member/agentUpdate");
    }
     ...
     * @return Ambigous <multitype:unknown, multitype:unknown unknown string >
     */
    public function memberAgentDelete(){
