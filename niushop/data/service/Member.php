...
use data\service\Member\MemberInvite as MemberInvite;

class Member extends User implements IMember{

    protected  $puid;
    protected $p_invite_code;
     ...
     * 验证邀请码是否存在
     */
    public function menberInvite(){
        $p_invite_code = request()->post('invite_code', 0);//邀请码
        if($p_invite_code){
            if(strlen($p_invite_code)==5) {
                $member = new NsMemberModel();
                $info = $member->getInfo(array('invite_code' => $p_invite_code), 'uid');
                if(!$info){
                    $retval = [
                        'code' => 1,
                        'message' => '注册邀请码长度为5'
                    ...
                    $this->p_invite_code = $p_invite_code;
                    $this->puid = $info['uid'];
                }
            }else{
                $retval = [
    ...
    public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid)
    {
        ...
        if ($res > 0) {
            ...
            $invite_code = runhook('Notify','createCode',$res);
            $data = array(
                'uid' => $res,
            ...
            // 关联邀请码用户
            $p_invite_code = request()->post('invite_code', '');//邀请码
            ...
            if($puid = $this->puid ) {
                ...
                $dataInvite = array(
                    'uid' => $res,
                    'pid' => $puid,
                    'p_invite_code' => $this->p_invite_code,
                );
                $memberInvite->addMemberInvite($dataInvite);
            }
            //
    ...
    /**
     * 通过用户id更新 is_system 可以系统登录
     *
     * @param unknown $uid
     * @param unknown $nickName
     */
    public function updateSystemByUid($uid)
    {
        $user = new UserModel();
        $result = $user->save([
            'is_system' => 1
