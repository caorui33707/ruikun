...
$(function(){
	...
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/config/helpdocument'); ?>",
			async : true,
			data : {
		...
		$( "#dialog" ).dialog({
	        buttons: {
	            "确定": function() {
	                ...
	                $.ajax({
	        			type : "post",
	        			url: "<?php echo __URL('PLATFORM_MAIN/config/titledelete'); ?>",
	        			async : true,
	        			data: { "id": id },
	        			success : function(data) {
	        				if(data["code"] > 0 ){
	        					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/helpdocument'); ?>");
	        				}else{
	        					showMessage('error', data["message"]);
	        				}
	        			}
