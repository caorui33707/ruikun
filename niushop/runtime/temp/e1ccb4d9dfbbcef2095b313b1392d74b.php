	...
	<div class="panel-body">
		<form class="form-horizontal" id="form" name="form" onsubmit="">
			<div class="form-group">
				<label class="col-sm-2 control-label"><span class="required">*</span>分类名称</label>
				<div class="col-md-4">
				...
				<div class="col-sm-4">
					<select id="pid" name="account" class="form-control m-b">
						<option value="0">顶级分类</option> 
						 <?php if(is_array($article_class_frist_list) || $article_class_frist_list instanceof \think\Collection || $article_class_frist_list instanceof \think\Paginator): $i = 0; $__LIST__ = $article_class_frist_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
...
function addArticleClassAjax(){
	...
	if(verify(name)){
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/Cms/addUpdateAritcleClass'); ?>",
			data : {
				'name' : name,
				'pid' : pid,
				'sort' : sort,
			...
			success : function(data) {
				if (data["code"] > 0) {
					showMessage('success', "添加成功","<?php echo __URL('PLATFORM_MAIN/Cms/articleClassList'); ?>");
				}else{
