...
<script>
	function setHotSearchAjax(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/config/hotsearchconfig'); ?>",
			data:{
				'keywords':keywords
			},
			async:true,
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/searchconfig'); ?>");
				}else{
	...
	function defaultHotSearchAjax(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/config/defaultsearchconfig'); ?>",
			data:{
