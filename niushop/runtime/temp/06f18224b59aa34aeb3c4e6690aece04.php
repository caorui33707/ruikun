								...
								<?php else: if(is_array($goods_collection_list) || $goods_collection_list instanceof \think\Collection || $goods_collection_list instanceof \think\Paginator): if( count($goods_collection_list)==0 ) : echo "" ;else: foreach($goods_collection_list as $k=>$goods): if(!(empty($goods['goods_id']) || (($goods['goods_id'] instanceof \think\Collection || $goods['goods_id'] instanceof \think\Paginator ) && $goods['goods_id']->isEmpty()))): ?>
											<li class="favorite-pic-list">
												<div class="favorite-goods-thumb">
													<a href="<?php echo __URL('SHOP_MAIN/goods/goodsinfo','goodsid='.$goods['goods_id']); ?>" target="_blank" title="<?php echo $goods['goods_name']; ?>"><img src="<?php echo __IMG($goods['pic_cover_mid']); ?>" alt="<?php echo $goods['goods_name']; ?>" width="200" height="200"></a>
												</div>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_25648__=1;$__FOR_END_25648__=$page_count+1;for($i=$__FOR_START_25648__;$i < $__FOR_END_25648__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_15731__=1;$__FOR_END_15731__=$page_num+1;for($i=$__FOR_START_15731__;$i < $__FOR_END_15731__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_30178__=$page_count-$page_num+1;$__FOR_END_30178__=$page_count+1;for($i=$__FOR_START_30178__;$i < $__FOR_END_30178__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_7853__=$page-($page_num-1)/2;$__FOR_END_7853__=$page+($page_num-1)/2+1;for($i=$__FOR_START_7853__;$i < $__FOR_END_7853__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
