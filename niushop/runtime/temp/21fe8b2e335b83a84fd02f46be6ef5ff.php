<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:39:"template/platform\Config\shopagant.html";i:1526376071;s:27:"template/platform\base.html";i:1513303781;s:44:"template/platform\controlCommonVariable.html";i:1513303781;s:31:"template/platform\urlModel.html";i:1513303781;s:27:"template/platform\page.html";i:1513303781;s:33:"template/platform\openDialog.html";i:1513303781;}*/ ?>
<!DOCTYPE html>
...
<div class="row padder-v">
	<div class="col-sm-9">
		<a href="PLATFORM_MAIN/Config/addAgant"
...
//分页数据
 function LoadingInfo(pageIndex) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/config/shopAgantList'); ?>",
		async : true,
		data : {
		...
		success : function(data) {
			...
			if (data["data"].length > 0) {
				for (var i = 0; i < data["data"].length; i++) {
			        ...
			        html += '<td class="center"><a href="'+__URL('PLATFORM_MAIN/Config/updateShopNavigation?nav_id='+ data['data'][i]['nav_id'])+'">修改</a> <a style="cursor: pointer;" onclick="deleteArticle('+data['data'][i]['nav_id']+')">删除</a></td> ';
				    html += '</tr>';
				}
			} else {
	...
	$( "#dialog" ).dialog({				
        buttons: {
            "确定": function() {
                ...
                $.ajax({
    				type : "post",
    				url : "<?php echo __URL('PLATFORM_MAIN/config/delshopnavigation'); ?>",
    				async : true,
    				data : {
    					"nav_id" : nav_id
    				},
...
function load(){
	$('.num').change(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/config/modifyshopnavigationsort'); ?>",
