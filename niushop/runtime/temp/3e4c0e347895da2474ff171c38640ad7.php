...
$(function(){
	...
	<input type="hidden" value="<?php echo $account; ?>" id="subscription"/>
	<div class="member-main">
		<div class="box" id="firstpage">
			<div class="tabmenu">
				<ul class="tab">
					<li class="active"><?php echo lang('member_my_points'); ?></li>
				</ul>
			</div>
			<div class="member-data-header">
				<span class="member-data-title">
					...
					<span>(<?php echo lang('goods_integral'); ?>:<span class="sum"><?php echo $sum; ?></span>)</span>
				</span>
				
				<div class="right-function">
					<?php if($account !=null && $account>0): ?>
					<a href="javascript:;" id="extract"><?php echo lang('member_exchange_balance'); ?></a>&nbsp;|
					<?php endif; ?>
					<a href="<?php echo __URL('SHOP_MAIN/member/integrallist'); ?>"><?php echo lang('member_my_points'); ?></a>
				</div>
			</div>
			<?php if($total_count != ''): ?>
			<table>
				<tr align="center">
					...
					<td><?php echo lang('goods_number'); ?></td>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_4956__=1;$__FOR_END_4956__=$page_count+1;for($i=$__FOR_START_4956__;$i < $__FOR_END_4956__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_15823__=1;$__FOR_END_15823__=$page_num+1;for($i=$__FOR_START_15823__;$i < $__FOR_END_15823__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_3630__=$page_count-$page_num+1;$__FOR_END_3630__=$page_count+1;for($i=$__FOR_START_3630__;$i < $__FOR_END_3630__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_30889__=$page-($page_num-1)/2;$__FOR_END_30889__=$page+($page_num-1)/2+1;for($i=$__FOR_START_30889__;$i < $__FOR_END_30889__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
...
<?php endif; else: ?>
			<div>
				<div class="emptyFrame"> <i class="user_point"></i><span><?php echo lang('member_no_scored_yet'); ?></span> </div>
			</div>
			<?php endif; ?>
		</div>
		<div class="box" id="secondpage" style="display:none;">
			...
			<div class="tabmenu">
				<ul class="tab">
					<li class="active"><?php echo lang('member_exchange_balance'); ?></li>
				</ul>
			</div>
			...
			<div class="member-data-header">
				...
				<div class="right-function">
					<a href="javascript:;" id="extract2"><?php echo lang('member_integration_details'); ?></a>&nbsp;|
					<a href="<?php echo __URL('SHOP_MAIN/member/integrallist'); ?>"><?php echo lang('member_my_points'); ?></a>
				</div>
			</div>
			<form name="formSurplus" method="post" action="<?php echo __URL('SHOP_MAIN/member/exchange'); ?>" id="formsubmit">
				<input type="hidden" name="shopid" value="<?php echo $shop_id; ?>" /> 
				<table width="100%" border="0" cellpadding="10" cellspacing="1" bgcolor="#eeeeee">
					<tr>
						<td width="10%" align="right"><?php echo lang('member_exchange_product_score'); ?>:</td>
						<td align="left">
							<input type="text" name="amount" class="inputBg" size="30" min="0" max="<?php echo $sum; ?>" style="width:86px;border:1px solid #ccc;"/>
							<span style="display:inline-block;" id="proving"></span>
						</td>
					</tr>
					<tr>
						<td width="10%" align="right"><span style="display:inline-block;"><?php echo lang('member_integral_available'); ?><span id="number"><?php echo $sum; ?></span></span></td>
						<?php if($account !=null && $account>0): ?>
						<td align="left"><?php echo lang('member_exchange_rate'); ?><?php echo $account; ?><?php echo lang('element'); ?></td>
						<?php else: ?>
						<td align="left"><?php echo lang('member_no_integral_change'); ?></td>
						<?php endif; ?>
					</tr>
					<tr>
						<td colspan="2" align="center">
							...
							<?php if($account!=null && $account>0): ?>
							<input type="submit" name="submit" class="common-btn" value="<?php echo lang('member_exchange'); ?>" />
							<input type="hidden" id="hidden_account" value="<?php echo $account; ?>" />
							<?php else: ?>
							<input type="submit" name="submit" class="common-btn-disabled" value="<?php echo lang('member_exchange'); ?>" disabled="disabled" />
							<?php endif; ?>
						</td>
					</tr>
				</table>
...
<input type="hidden" value="<?php echo $sum; ?>" id="hidden_sum_integral" />
...
$(document).ready(function(){
	...
	$("#extract").on("click",function(){
		number = $("#hidden_sum_integral").val();
		$("#firstpage").hide();
		$("#secondpage").show();
	});
	//点击返回余额明细界面
	...
	$("input[name='amount']").on("keyup",function(){
		...
		if(subscription == 0){
			...
			$("#proving").text("<?php echo lang('member_amend_exchange'); ?>").css("color","red");
			yz = false;
			is_ok
		}else if(!re.test(getnumber)){
			$(this).css("border","1px solid red");
			$("#proving").text("<?php echo lang('member_integral_must_integer'); ?>").css("color","red");
			yz = false;
			...
			if(stringToNumber>0 && stringToNumber<=max){
				...
				$("#proving").text("<?php echo lang('member_amount_exchange'); ?>¥"+result.toFixed(2)).css("color","red");
				yz = true;
			}else{
				$(this).css("border","1px solid red");
				$("#proving").text("<?php echo lang('member_maximum_convertibility'); ?>"+number+"<?php echo lang('goods_integral'); ?>").css("color","red");
				yz = false;
			}
		}else{
			$(this).css("border","1px solid red");
			$("#proving").text("<?php echo lang('member_correct_integral_format'); ?>").css("color","red");
			yz = false;
		}
	});
	$("input[name='submit']").on("click",function(){
		if($("#hidden_account").val() == undefined || $("#hidden_account").val() == "" || parseFloat($("#hidden_account").val()) == 0){
			$.msg("<?php echo lang('member_no_balance_installed'); ?>");
			return false;
		}
		if(yz){
			$("#formsubmit").submit();
