...
function getDataBaselist(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/dbdatabase/databaselist'); ?>",
		success : function(data) {
			var html = '';
		...
		buttons: {
			"确定": function() {
				$(this).dialog('close');
					$.ajax({
						type : "post",
						url : "<?php echo __URL('PLATFORM_MAIN/dbdatabase/exportdatabase'); ?>",
						data : { "tables" : _tables },
						dataType : "json",
						success : function(data) {
...
function tableOperation(type, tab){
	...
	if(!flag){
		...
		if(type == 1){
			...
			$.ajax({
				type:"post",
				url:"<?php echo __URL('PLATFORM_MAIN/dbdatabase/repair'); ?>",
				data:{tables:tab},
				success: function (data) {
					flag = false;
					if(data.status > 0){
			...
			$.ajax({
				type:"post",
				url:"<?php echo __URL('PLATFORM_MAIN/dbdatabase/optimize'); ?>",
				data:{tables:tab},
				success: function (data) {
					flag = false;
					if(data.status > 0){
