...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/shop/shoplist'); ?>",
			async : true,
			data : {
			...
			success : function(data) {
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="center"><a href="'+__URL('PLATFORM_MAIN/Shop/updateShop?shop_id='+ data['data'][i]['shop_id'])+'">修改</a>&nbsp;&nbsp;';
						if(data["data"][i]['shop_state'] == 1){
	...
	*/
	function load(shop_id,index){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __Url('PLATFORM_MAIN/shop/updateshoplist'); ?>",
	...
	*/
	function closeStatus(type,shop_id){
	                ...
	                $.ajax({
	        			type:"post",
	        			url:"<?php echo __Url('PLATFORM_MAIN/shop/setstatus'); ?>",
