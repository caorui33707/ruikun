	...
	<div class="panel-body">
		<form class="form-horizontal" id="form" name="form">
			<div class="form-group">
				...
				<div class="col-md-2">
					<input type="text" class="form-control" id="group_name" name="group_name" value="<?php echo $info['group_name']; ?>" required>
				</div>
			...
			<div class="form-group level_0">
				...
				<?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): if( count($list)==0 ) : echo "" ;else: foreach($list as $key=>$v1): ?>
					<div class="form-group level_1">
						<div class="col-md-1">
							<label class="checkbox-inline"> <input type="checkbox" dir="0" onclick="selectlevel('level_1', this)" name="module_id" value="<?php echo $v1['module_id']; ?>" <?php if(in_array(($v1['module_id']), is_array($info['module_id_array'])?$info['module_id_array']:explode(',',$info['module_id_array']))): ?>checked<?php endif; ?>> <b style="color:#555;"><?php echo $v1['module_name']; ?></b></label>
						</div>
						<div class="col-md-11">
							...
							<div class="form-group level_2">
								<div class="col-md-2">
									<label class="checkbox-inline"> <input type="checkbox" dir="<?php echo $v2['pid']; ?>" onclick="selectlevel('level_2',this)" name="module_id" value="<?php echo $v2['module_id']; ?>" <?php if(in_array(($v2['module_id']), is_array($info['module_id_array'])?$info['module_id_array']:explode(',',$info['module_id_array']))): ?>checked<?php endif; ?>> <b><?php echo $v2['module_name']; ?></b></label>
								</div>
								<div class="col-md-10">
									<?php if(is_array($v2['child']) || $v2['child'] instanceof \think\Collection || $v2['child'] instanceof \think\Paginator): if( count($v2['child'])==0 ) : echo "" ;else: foreach($v2['child'] as $key=>$v3): ?>
										<label class="checkbox-inline"> <input type="checkbox" parentdir="<?php echo $v2['pid']; ?>" dir="<?php echo $v3['pid']; ?>" onclick="selectlevel('level_3',this)" name="module_id" value="<?php echo $v3['module_id']; ?>" <?php if(in_array(($v3['module_id']), is_array($info['module_id_array'])?$info['module_id_array']:explode(',',$info['module_id_array']))): ?>checked<?php endif; ?>> <?php echo $v3['module_name']; ?></label>
									<?php endforeach; endif; else: echo "" ;endif; ?>
								</div>
							</div>
							<?php endforeach; endif; else: echo "" ;endif; ?>
	...
	function updateAuthGroupAjax() {
		...
		$.ajax({
			...
			success : function(data) {
				if (data['code'] > 0) {
					showMessage('success', "修改成功","<?php echo __URL('PLATFORM_MAIN/auth/authgrouplist'); ?>");
				}else{
