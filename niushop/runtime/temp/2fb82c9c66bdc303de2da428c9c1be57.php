...
function addGoodsSpec() {
	location.href = "<?php echo __URL('PLATFORM_MAIN/goods/addgoodsspec'); ?>";
}
$(function(){
	LoadingInfo(1);
});
			...
			buttons: {
				"确定": function() {
					...
					$.ajax({
						...
						success : function(data) {
							if (data['code'] > 0) {
	 							showMessage('success', data["message"], "<?php echo __URL('PLATFORM_MAIN/goods/goodsspeclist'); ?>");
							}else if(data['code'] == 0){
	 							showMessage('error', data["message"]);
							}else if(data['code'] == -1){
