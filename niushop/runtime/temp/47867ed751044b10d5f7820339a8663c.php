...
<div class="modal" id="add_user" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="modal-infp-style" style="height:370px;overflow:auto;">
...
// 				alert(JSON.stringify(data));
				...
				if (data["data"].length > 0) {
					for (var i = 0; i < data["data"].length; i++) {
						...
						html += '<td class="tal">' +'注册时间 : '+ timeStampTurnTime(data["data"][i]["reg_time"]) +'<br>'+'最后登录 : '+ timeStampTurnTime(data["data"][i]["current_login_time"])+'</td>';
						html += data["data"][i]["user_status"] == 0 ? '<td style="color:red;">锁定</td>' : '<td style="color:green;">正常</td>';
						html += '<td><a href="'+ __URL('PLATFORM_MAIN/member/pointdetail?member_id='+ data['data'][i]['uid']) +'">积分明细</a>&nbsp;&nbsp;';
						html += '<a href="' +  __URL('PLATFORM_MAIN/member/accountdetail?member_id='+ data['data'][i]['uid']) +'">余额明细</a><br/>';
...
function addUser(){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/member/addmember'); ?>",
		data : {
			'username' : username,
			'nickname' :nickname,
			'password' : password,
		...
		success : function(data) {
			if (data['code'] > 0) {
				showMessage('success',data['message'],"<?php echo __URL('PLATFORM_MAIN/member/memberlist'); ?>");
				$("#add_user").modal("hide");
// 				LoadingInfo(getCurrentIndex(1,'#productTbody'));
	...
	function addRechargeCoin(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/Member/addRechargeCoin'); ?>",
			data:{
				'id' : uid,
				'num' : num,
				'text' : text
	...
	function modify_user(uid){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/member/getmemberdetail'); ?>",
			data : { 'uid':id, },
			success : function(data) {
	...
	*/
	function modifypassword(){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/member/updatememberpassword'); ?>",
			data : {
				'uid':uid,
			...
			success : function(data) {
				if (data['code'] > 0) {
					showMessage('success','修改成功',"<?php echo __URL('PLATFORM_MAIN/member/memberlist'); ?>");
		...
		$( "#dialog" ).dialog({
			buttons: {
				"确定": function() {
					$.ajax({
						type : "post",
						url : "<?php echo __URL('PLATFORM_MAIN/member/deletemember'); ?>",
...
// 	//修改会员
	function modifyUser(){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/member/updatemember'); ?>",
			data : {
				'uid':uid,
				'user_name' :username,
				'nick_name' : nickname,
