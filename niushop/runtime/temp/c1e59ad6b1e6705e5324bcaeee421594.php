<?php if (!defined('THINK_PATH')) exit(); /*a:7:{s:43:"template/wap\default\Goods\goodsDetail.html";i:1513303782;s:30:"template/wap\default\base.html";i:1513303782;s:34:"template/wap\default\urlModel.html";i:1513303783;s:45:"template/wap\default\Goods\controlDetail.html";i:1513303782;s:42:"template/wap\default\controlCopyRight.html";i:1513303782;s:39:"template/wap\default\shareContents.html";i:1513303783;s:31:"template/wap\default\share.html";i:1513303783;}*/ ?>
<!DOCTYPE html>
<html>
<head>
...
<div class="buy_area">
	...
	<div class="stock-detail" >
		<span style="display:inline-block;width:50%;">销量:<?php echo $goods_detail['sales']; ?>笔</span>
		<span class="js-shipping-fee-name"></span><!-- 运费 -->
	</div>
</div>
<hr class="with-margin-l" />
<hr class="divider-line" />
...
<div class="price_postage">
	<span class="price_postage_tit single"><i style="color: #fff;font-size: 12px;padding: 3px;background: #f44;border-radius: 2px;"><?php echo lang('goods_free_shipping'); ?></i>&nbsp;<?php echo $goods_detail['baoyou_name']; ?></span>
</div>
<hr class="with-margin-l" />
<?php endif; if($goods_detail['baoyou_name'] != '' || $goods_detail['mansong_name'] != ''): ?>
<hr class="divider-line" />
...
<div class="statistics">
		<div class="app-field clearfix">
			<div class="control-group">
				<div class="custom-store block-item">
					<a class="custom-store-link clearfix" href="<?php echo __URL('APP_MAIN/shop/index?shop_id='.$goods_detail['shop_id']); ?>">
	...
	<div class="mod_tab_wrap">
		<div class="mod_tab" id="detailTab">
			...
			<span onclick="showProperty(this,3)"><?php echo lang('goods_commodity_evaluation'); ?>(<?php echo $evaluates_count['evaluate_count']; ?>)</span>
		</div>
	</div>
			...
			<li class="s-buy-pro">
				<span class="pro-img">
					...
					<?php if(!(empty($goods_detail['sku_picture_list']) || (($goods_detail['sku_picture_list'] instanceof \think\Collection || $goods_detail['sku_picture_list'] instanceof \think\Paginator ) && $goods_detail['sku_picture_list']->isEmpty()))): if(is_array($goods_detail['sku_picture_list']) || $goods_detail['sku_picture_list'] instanceof \think\Collection || $goods_detail['sku_picture_list'] instanceof \think\Paginator): if( count($goods_detail['sku_picture_list'])==0 ) : echo "" ;else: foreach($goods_detail['sku_picture_list'] as $key=>$img): if(is_array($img['album_picture_list']) || $img['album_picture_list'] instanceof \think\Collection || $img['album_picture_list'] instanceof \think\Paginator): if( count($img['album_picture_list'])==0 ) : echo "" ;else: foreach($img['album_picture_list'] as $k=>$p): ?>
								<input type="hidden" id="spec_picture_id<?php echo $p['pic_id']; ?>" value="<?php echo __IMG($p['pic_cover_micro']); ?>" />
		...
		<ul class="s-buy-ul specification" id="specification" style="z-index: 9000;">
			...
			<li class="s-buy-li" >
				...
				<?php if(is_array($spec['value']) || $spec['value'] instanceof \think\Collection || $spec['value'] instanceof \think\Paginator): if( count($spec['value'])==0 ) : echo "" ;else: foreach($spec['value'] as $key=>$spec_value): if($spec_value['spec_id'] == $spec['spec_id']): ?>
						<button class="select" name="Span<?php echo $k; ?>" id="<?php echo $spec_value['spec_id']; ?>:<?php echo $spec_value['spec_value_id']; ?>" onclick="change(this);" data-picture-id="<?php echo $spec_value['picture']; ?>">
			...
			for(var i=0;i<data['data'].length;i++){
				...
				listhtml+=' <div class="tablelist" style="border-bottom: 1px solid #e5e5e5;margin-bottom: 10px;">'
					...
					if(dataitem["user_img"] != "" && dataitem['user_img'] != undefined){
						listhtml+='<img src="'+__IMG(dataitem['user_img'])+'" style="width:35px;border-radius: 50%;">';
					}else{
						listhtml+='<img src="__TEMP__/<?php echo $style; ?>/public/images/default_user_portrait_0.png" style="width:35px;border-radius: 50%;"/>';
					}
					listhtml+='</div>'
