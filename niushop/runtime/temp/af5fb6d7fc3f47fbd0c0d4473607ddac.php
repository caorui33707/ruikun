...
//查询用户列表
	function LoadingInfo(pageIndex) {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/config/blockList'); ?>",
			async : true,
			data : {
	...
	$( "#dialog" ).dialog({
        buttons: {
            "确定": function() {
                ...
                $.ajax({
        			type : "post",
        			url : "<?php echo __URL('PLATFORM_MAIN/config/delblock'); ?>",
        			async : true,
        			data : {
        				"block_id" : block_id
        			},
        			success : function(data) {
        				if(data["code"] > 0 ){
        					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/blocklist'); ?>");
        				}else{
...
function setWebBlockIsBlock(eve,block_id,is_block){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/config/setwebblockisblock'); ?>",
		async : true,
		data : {
			"block_id" : block_id,
			"is_display" : is_block
