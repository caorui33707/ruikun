...
//         }
     ...
     * 获取推荐手机设置
     */
    public function getRecommendMobile($instanceid){
        $info = $this->config_module->getInfo([
            'key' => $this->key ,
            'instance_id' => $instanceid
        ], 'is_use,value');
     ...
     * 添加和更新推荐手机设置
     */
    public function updateRecommendMobile($instanceid,$is_use){
        $info = $this->config_module->getInfo([
            'key' => $this->key ,
            'instance_id' => $instanceid
        ], 'value, is_use');
        ...
        if (empty($info['value'])) {
            $data['key'] = $this->key;
            $data['instance_id'] = $instanceid;
           $r =  $this->config_module->addInfo($data);
        } else {

            $r =  $this->config_module->updateInfo([
                                            'key' => $this->key ,
                                            'instance_id' => $instanceid
                                        ],
                                    $data);
...
//    public function getRecommendMobileKey($instanceid){
//            $this->key = 'RecommendMobile';
//    }

    public function setRecommendMobileKey(){
        $this->key = 'RecommendMobile';
    }
    ...
    public function saveAgentMobile($instanceid,$is_use,$val){
        $info = $this->config_module->getInfo([
            'key' => 'AGENTMOBILE' ,
            'instance_id' => $instanceid
        ], 'value, is_use');
     ...
     * 获取代理商推荐人设置配置
     */
    public function getAgentMobile($instanceid){
        $info = $this->config_module->getInfo([
            'key' => 'AGENTMOBILE' ,
            'instance_id' => $instanceid
        ], 'is_use,value');
