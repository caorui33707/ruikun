...
//  		verify(ad_image, link_url);
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/config/addlink'); ?>",
			data : {
			...
			success : function(data) {
				if (data["code"] > 0) {
					showMessage('success', "添加成功","<?php echo __URL('PLATFORM_MAIN/config/linklist'); ?>");
				}else{
