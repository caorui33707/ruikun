...
$("#integrat_money").on("keyup",function(){
  ...
  function SavepointConfig(){
	  ...
	  var desc = $("#editor").html();
		$.ajax({
			type:"post",
			url : "<?php echo __URL('PLATFORM_MAIN/config/pointconfig'); ?>",
			async : true,
			data : {
			...
			success : function(data){
				if(data['code'] > 0)
					{
						showMessage('success', data["message"], "<?php echo __URL('PLATFORM_MAIN/config/pointconfig'); ?>");
					}else{
