							...
							<?php endif; if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?>
								...
								<!--  onclick="location.href='SHOP_MAIN/shop/shopindex?shop_id=<?php echo $vo['shop_id']; ?>';" style="cursor: pointer;" -->
									<div class="favorite-goods-thumb" >
										...
										<a href="<?php echo __URL('SHOP_MAIN/shop/shopindex?shop_id='.$vo['shop_id']); ?>" target="_blank" title="<?php echo $vo['shop_name']; ?>">
									...
									<div class="favorite-goods-info">
										<dl>
											<dt>
												<a href="<?php echo __URL('SHOP_MAIN/shop/shopindex?shop_id='.$vo['shop_id']); ?>" target="_blank" title="<?php echo $vo['shop_name']; ?>"><?php echo $vo['shop_name']; ?></a>
											</dt>
											<dd class="price"><?php echo $vo['shop_name']; ?></dd>
											<dd class="hover_tan" onclick="javascript:cancelCollection(<?php echo $vo['shop_id']; ?>,'shop',true);">
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_23752__=1;$__FOR_END_23752__=$page_count+1;for($i=$__FOR_START_23752__;$i < $__FOR_END_23752__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_22891__=1;$__FOR_END_22891__=$page_num+1;for($i=$__FOR_START_22891__;$i < $__FOR_END_22891__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_9914__=$page_count-$page_num+1;$__FOR_END_9914__=$page_count+1;for($i=$__FOR_START_9914__;$i < $__FOR_END_9914__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_11237__=$page-($page_num-1)/2;$__FOR_END_11237__=$page+($page_num-1)/2+1;for($i=$__FOR_START_11237__;$i < $__FOR_END_11237__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
