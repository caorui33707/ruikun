	...
	<div class="panel-body">
		<form class="form-horizontal">
           ...
           <div class="line line-dashed line-lg pull-in"></div>
			...
			<div class="line line-dashed line-lg pull-in"></div>
			<div class="form-group">
				<label class="col-sm-2 control-label">网站访问模式</label>
				<div class="col-sm-4">
					<select id="visit_pattern" name="visit_pattern"class="form-control m-b">
				...
				<div class="col-sm-4">
					<select id="style_id" name="account" class="form-control m-b">
						<?php if(is_array($style_list) || $style_list instanceof \think\Collection || $style_list instanceof \think\Paginator): $i = 0; $__LIST__ = $style_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
							<option value="<?php echo $v['style_id']; ?>" <?php if($website['style_id_pc'] == $v['style_id']): ?> selected<?php endif; ?>><?php echo $v['style_name']; ?></option>
						<?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
			</div>
		...
		var visit_pattern = $("#visit_pattern").val();
		 $.ajax({
			...
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/config/webconfig'); ?>");
				}else{
