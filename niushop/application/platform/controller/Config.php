...
use data\model\SysAgentModel as SysAgentModel;
use data\model\SysAgentCompanyModel as sysAgentCompanyModel;
/**
 * 网站设置模块控制器
 *
...
/**
    ...
    public function recommendMobile(){
        ...
        $config->setRecommendMobileKey();
        if(request()->isAjax()){
            $is_use = request()->get('is_use', 0);
            if( ! $s = $config ->updateRecommendMobile($this->instance_id,$is_use) ){
                return AjaxReturn(1);
             }else{
                return false;
            }
        ...
        $recommend_mobile = $config->getRecommendMobile($this->instance_id);
        $this->assign('recommend_mobile', $recommend_mobile);

        return view($this->style . 'Config/recommendmobile');
    }

    /**
     *  添加
     */
    public function addShopAgant(){
        if(request()->isAjax()){
            $sysAgentObj = new  SysAgentModel();
            $objCompany = new  sysAgentCompanyModel();
            $data = request()->post();
            $data['level'] = $this->getAgantLevel();
            $data['uid'] = $this->uid;
            ...
            if ($agent_type == 2 ) { // 公司
                ...
                unset($data['business_licence_number']);
                ...
                $objCompany->save($dataCompany);
            }

            $res = $sysAgentObj->save($data);

            $arr = $sysAgentObj->getInfo(array('uid'=>$this->uid),'id');

            $objCompany->save(array('agent_id'=>$arr['id']),array('uid'=>$this->uid));

            return AjaxReturn($res);

        }
        return view($this->style . 'Config/agantshopadd');
    }
 ...
 */
    public function updateStatus(){
        if(request()->isAjax()){
            ...
            $data = request()->post();
            $data['level'] = $this->getAgantLevel();
            $res = $sysAgentObj->save($data,array('id'=>$id));
            return AjaxReturn($res);
     ...
     *  更新
     */
    public function updateShopAgant(){
        $sysAgentObj = new  SysAgentModel();
        $objCompany = new  sysAgentCompanyModel();
        ...
        if(request()->isAjax()) {
            ...
            $dataobj  = $sysAgentObj->getInfo(array('id'=>$id));
            $is_status = $dataobj['is_status'];

            if($is_status == 3){ // 已审核通过
                return AjaxReturn(0);
            }

            $dataAjax = request()->post();
            $dataAjax['level'] = $this->getAgantLevel();
            if (!$this->is_admin) { // 不是系统管修改uid
                $data['uid'] = $this->uid;
            }

            $res = $sysAgentObj->save($dataAjax,array('id'=>$id));
        ...
        $id = request()->get('id');
        $data  = $sysAgentObj->getInfo(array('id'=>$id));

        $dataCompanyInfo = $objCompany->getInfo(array('agent_id'=>$id));

        $dataCompanyInfo = $dataCompanyInfo?$dataCompanyInfo:array();

        $data = array_merge($data,$dataCompanyInfo);

        $this->assign('data', $data);

        if($this->is_admin){
            return view($this->style . 'Config/agantshopAdminDetails');
        }

        return view($this->style . 'Config/agantshopupdate');
    }


    public function updateCompany(){
        $obj = new  sysAgentCompanyModel();
        if(request()->isAjax()){
            $id = request()->post('id');
            $data = request()->post();
            $res = $obj->save($data,array('id'=>$id));
            return AjaxReturn($res);
...
//            }
    ...
    public function shopAgantAdminList(){
        if(request()->isAjax()){
            $sysAgentObj = new  SysAgentModel();
            $page_index = $this->request->post('pageIndex');
            $data = $sysAgentObj->pageQuery($page_index , $page_size = 5,['is_default'=>0],'','*');
          return $data;
        }
     ...
     * 列表
     */
    public function shopAgantList(){
        if(request()->isAjax()){
            $sysAgentObj = new  SysAgentModel();
            $condition = array('uid'=>$this->uid,'is_default'=>0);
            $data = $sysAgentObj->getInfo($condition);
        ...
        if($this->is_admin){
            ...
            return view($this->style  . 'Config/agantshopAdmin');
        }

        return view($this->style  . 'Config/agantshop');
    }
    ...
    public function agentSave(){
        ...
        if(request()->isAjax()){
            $is_use = request()->get('is_use', 0);
            $value = request()->post();
            if(  $s = $config ->saveAgentMobile($this->instance_id,$is_use,$value) ){
                return AjaxReturn(1);
            }else{
                return false;
            }
        ...
        $agentList= $config->getAgentMobile($this->instance_id);
        $this->assign('agent_list', $agentList);


        return view($this->style  . 'Config/agentsave');
    }

    public function getAgantLevel(){
        $district_id = request()->post('district_id');
        $city_id = request()->post('city_id');
        $province_id = request()->post('province_id');
        if($district_id>0){
