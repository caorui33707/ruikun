 ...
 */
	function setWpayConfigAjax() {
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('PLATFORM_MAIN/Config/setwpayconfig'); ?>",
			data : {
				'appkey' : appkey,
				'appsecret' : appsecret,
 ...
 */
	function setAliPayConfigAjax(){
		...
		$.ajax({
			...
			success:function (data) {
				if (data["code"] > 0) {
					showMessage('success', data["message"],"<?php echo __URL('PLATFORM_MAIN/Config/payAliconfig'); ?>");
				}else{
					showMessage('error', data["message"]);
				}	
			}
