...
$(function(){
	...
	<div class="member-main">
		<div class="box">
			...
			<?php if($total_count == ''): ?>
				<div class="tabmenu">
						<input type="button" value="新增代理商" onclick="location.href='<?php echo __URL('SHOP_MAIN/member/agentinsert'); ?>'" class="main-btn common-btn" />

				</div>
			<?php endif; if($total_count != ''): ?>
			<div>
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="bought-table">
					<thead>
						<tr class="col-name">
							...
							<th width="15%" style="border-right: 1px solid #E6E6E6;"><?php echo lang('goods_operation'); ?></th>
						</tr>
					</thead>
					<tbody class="close-order" id="list_address">
						<tr class="order-bd last">
							<td align="center" class="baobei no-border-right order_goods_info"><div class="last"><?php echo $list['agent_name']; ?></div></td>
							<td align="center" class="baobei no-border-right order_goods_info"><div class="last"><?php echo $list['identity_card']; ?></div></td>
							<td align="center" class="baobei no-border-right order_goods_info"><div class="last"><img width="120px" height="200px" src="<?php echo $list['identity_card_photo']; ?>" /></div></td>
							<td align="center" class="baobei no-border-right order_goods_info"><div class="last"><?php echo $list['linkman_name']; ?></div></td>
							<td align="center" class="baobei no-border-right order_goods_info"><div class="last"><?php echo $list['agent_info']; ?></div></td>
							<td align="center" class="baobei no-border-right order_goods_info">
								<div class="last">
									<?php echo $list['status_val']; ?>
								</div>
							</td>
							<td align="center" class="baobei no-border-right order_goods_info">
								<div class="last">
									<?php if($list['is_status'] == 1): ?>
									 <a href="<?php echo __URL('SHOP_MAIN/member/agentUpdate','id='.$list['id']); ?>" class="main-btn main-btn-large"><?php echo lang('member_modify'); ?></a>
									<a href="javascript:void(0);" onclick="agent_delete(<?php echo $list['id']; ?>)" class="main-btn main-btn-large"><?php echo lang('goods_delete'); ?></a>
									<?php endif; ?>
								</div>
							</td>
						</tr>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_9543__=1;$__FOR_END_9543__=$page_count+1;for($i=$__FOR_START_9543__;$i < $__FOR_END_9543__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_2182__=1;$__FOR_END_2182__=$page_num+1;for($i=$__FOR_START_2182__;$i < $__FOR_END_2182__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_1377__=$page_count-$page_num+1;$__FOR_END_1377__=$page_count+1;for($i=$__FOR_START_1377__;$i < $__FOR_END_1377__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_20680__=$page-($page_num-1)/2;$__FOR_END_20680__=$page+($page_num-1)/2+1;for($i=$__FOR_START_20680__;$i < $__FOR_END_20680__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info.'?page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
...
function agent_delete(id){
	$.ajax({
		url: "<?php echo __URL('SHOP_MAIN/member/memberAgentDelete'); ?>",
		data: { "id": id },
		type: "post",
		success: function (res) {
			if (res["code"] == 1) {
