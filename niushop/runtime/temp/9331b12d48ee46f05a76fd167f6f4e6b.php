	...
	<!-- 			<li class="hidden-xs">
		...
		<div id="content" class="main">
              ...
              <div class="row padder-v">
                <div class="col-sm-9">
                  <a class="btn btn-sm btn-primary" href="<?php echo __URL('PLATFORM_MAIN/Cms/addArticle'); ?>" ><i class="fa fa-plus"></i>&nbsp;添加文章</a>
                </div>
                <div class="col-sm-2">
                  <div class="input-group">
...
function LoadingInfo(pageIndex) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('PLATFORM_MAIN/Cms/articleList'); ?>",
		async : true,
		data : {
		...
		success : function(data) {
			...
			if (data["data"].length > 0) {
				for (var i = 0; i < data["data"].length; i++) {
			        ...
			        html += '<td class="center"><a target="_blank" href="'+__URL('SHOP_MAIN/cms/articleclassinfo?article_id='+ data['data'][i]['article_id'])+'">预览</a>&nbsp;&nbsp;<a href="PLATFORM_MAIN/Cms/updateArticle?id='+ data['data'][i]['article_id']+'">修改</a>&nbsp;&nbsp;<a style="cursor: pointer;" onclick="deleteArticle('+data['data'][i]['article_id']+')">删除</a></td> ';
				    html += '</tr>';
				}
			} else {
				html += '<tr align="center"><th colspan="9" style="text-align:center">暂无文章</th></tr>';
			}
			$("#list").html(html);
	...
	$( "#dialog" ).dialog({				
        buttons: {
            "确定": function() {
                ...
                $.ajax({
    				type : "post",
    				url : "<?php echo __URL('PLATFORM_MAIN/Cms/deleteArticle'); ?>",
    				async : true,
    				data : {
...
function load(){
	$('.num').change(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('PLATFORM_MAIN/Cms/modifyArticleField'); ?>",
