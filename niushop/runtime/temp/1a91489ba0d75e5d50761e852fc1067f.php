...
<script>
    ...
    <div class="user-right" >
    	<div class="box" id="firstpage">
    		<div class="tabmenu">
				<ul class="tab pngFix">
					<li class="first active"><?php echo lang('member_integration_details'); ?></li>
				</ul>
			</div>
<!--            <span class="integral_title" style="display:inline-block;"><b><?php echo $shopname; ?>积分操作明细(可用积分:<span id="sum"><?php echo $sum; ?></span>)</b></span> -->
           	<div class="interal_table" style="height:40px;">
                ...
                <?php if(!isset($shopname)): ?>
                	...
                	<?php echo $shopname; endif; ?><span style="color:#989191;">(<?php echo lang('goods_integral'); ?>:<span id="sum"><?php echo $sum; ?></span>)</span>
                </span>
                 <span style="display:inline-block; float:right;font-size:14px;margin:10px;">
              		<?php if($account !=null && $account>0): ?>
              		<a class="main-color" id="extract" style="color:red;"><?php echo lang('member_exchange_balance'); ?></a> |
              		<?php endif; ?> 
                 <a href="<?php echo __URL('SHOP_MAIN/member/integrallist'); ?>" class="table_manual" style="color:#000;"><?php echo lang('member_my_points'); ?></a>
                 </span>
            </div> 
<?php if($total_count != ''): ?>
		<table>
...
<div id="pagination" class="page" >
	...
	<div class="page-num fr">
		...
		<div id="page_list" style="float: left;<?php echo $path_info; ?>">
			...
			<!-- 总页数小于总页码时就都显示 -->
			<?php if($page_count <= $page_num): $__FOR_START_28859__=1;$__FOR_END_28859__=$page_count+1;for($i=$__FOR_START_28859__;$i < $__FOR_END_28859__;$i+=1){ if($i == $page): ?>
			...
			<?php elseif($page <= ($page_num-1)/2): $__FOR_START_18762__=1;$__FOR_END_18762__=$page_num+1;for($i=$__FOR_START_18762__;$i < $__FOR_END_18762__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info,'page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 如果总页数等于当前页 或者 总页数小于当前页加上页码总数平均值显示 -->
			<?php elseif($page_count == $page or $page_count <= $page+($page_num-1)/2): $__FOR_START_32437__=$page_count-$page_num+1;$__FOR_END_32437__=$page_count+1;for($i=$__FOR_START_32437__;$i < $__FOR_END_32437__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info,'page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } ?>
			<!-- 否则就正常显示 -->
			<?php else: $__FOR_START_19180__=$page-($page_num-1)/2;$__FOR_END_19180__=$page+($page_num-1)/2+1;for($i=$__FOR_START_19180__;$i < $__FOR_END_19180__;$i+=1){ if($i == $page): ?>
						<span class="num curr"><a href="javascript:;" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php else: ?>
						<span class="num"><a href="<?php echo __URL('SHOP_MAIN/'.$path_info,'page='.$i.$query_string); ?>" data-cur-page="<?php echo $i; ?>"><?php echo $i; ?></a></span>
					<?php endif; } endif; ?>
...
<?php endif; ?>
        ...
        <div class="interal_table" style="height:40px;">
                <span style="display:inline-block; float:left;font-size:16px;margin:10px;">
                		<?php echo lang('goods_integral'); ?>
                </span>
                <span style="display:inline-block; float:right;font-size:14px;margin:10px;">
              	<a  class="main-color" id="extract2" style="color:red;"><?php echo lang('member_integration_details'); ?></a> | 
                 <a href="<?php echo __URL('SHOP_MAIN/member/integrallist'); ?>" class="table_manual" style="color:#000;"><?php echo lang('member_my_points'); ?></a>
                 </span>
            </div> 
           ...
           <input type="hidden" name="shopid" value="<?php echo $shop_id; ?>"> 
            <table width="100%" border="0" cellpadding="10" cellspacing="1" bgcolor="#eeeeee">
              <tr>
                ...
                <td align="left">
                	<input type="text" name="amount" value="" class="deal" size="30" min="0" max="<?php echo $sum; ?>" style="width:86px;border:1px solid #ccc;"/>
              ...
              <tr>
                <td colspan="2" align="center"><input type="hidden" name="surplus_type" value="1" />
                  <input type="hidden" name="act" value="act_account" />
                	<?php if($account!=null && $account>0): ?>
                  <input type="submit" name="submit"  class="main-btn main-btn-large" value="<?php echo lang('member_exchange'); ?>" />
					<input type="hidden" id="hidden_account" value="<?php echo $account; ?>" />
					<?php else: ?>
                  <input type="submit" name="submit" class="main-btn main-btn-large" value="<?php echo lang('member_exchange'); ?>" style="background: #BBBFC1;" disabled="disabled" />
					<?php endif; ?>
                  </td>
              </tr>
            </table>
